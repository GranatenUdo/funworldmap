...
describe('readHistory on corruption', () => {
  ...
  it('captures a Sentry exception when JSON.parse fails', () => {
    ...
    const [err, ctx] = (Sentry.captureException as ReturnType<typeof vi.fn>).mock.calls[0]
  ...
  it('captures a Sentry exception when version is unknown', () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ version: 99, days: {}, streak: { current: 0, longest: 0, lastActiveDate: null, lastMilestoneShown: 0 } }))
    readHistory()
    expect(Sentry.captureException).toHaveBeenCalledTimes(1)
...
describe('readResume on corruption', () => {
  ...
  it('captures unknown-version on resume version mismatch', () => {
    localStorage.setItem(RESUME_KEY, JSON.stringify({ version: 99, date: '2026-05-12', modeId: 'country-pinning', attempts: [] }))
    expect(readResume()).toBeNull()
    expect(Sentry.captureException).toHaveBeenCalledTimes(1)
    const [, ctx] = (Sentry.captureException as ReturnType<typeof vi.fn>).mock.calls[0]
    expect((ctx as { tags?: { area?: string; kind?: string } }).tags?.kind).toBe('unknown-version')
  ...
  it('does NOT capture on stale-date (benign)', () => {
    ...
    localStorage.setItem(RESUME_KEY, JSON.stringify({ version: 1, date: isoDate, modeId: 'country-pinning', attempts: [] }))
    expect(readResume()).toBeNull()
    expect(Sentry.captureException).not.toHaveBeenCalled()
  })
