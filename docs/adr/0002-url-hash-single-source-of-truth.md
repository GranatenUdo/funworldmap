...
writes the hash; all consuming components react to `hashchange`. Components hold no independent selection state; `useSelectedCountry`
re-derives from the hash on every `hashchange` and applies deselection itself,
since deselect clears the hash via `history.replaceState`, which fires no
`hashchange` event. Parsing and serialization live in `src/lib/hashState.ts`
(`parseHash` / `writeHash`).
