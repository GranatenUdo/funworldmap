...
- [x] `git branch --show-current` → `fix/2026-07-10-review-batch-1`; `git status --short` clean
- [x] `npm run check` green before starting (baseline)
- [x] No stray dev servers: `Get-NetTCPConnection -LocalPort 5173 -State Listen` empty

## Task 1 — tooltip overflow + stale hover artifacts

- [x] `src/index.css` — in the `.country-tooltip` rule add, after `position: absolute;`:
...
- [x] `src/hooks/useMapInteractions.ts` — extract the hover-clearing body shared by `mouseleave`
...
- [x] Register/unregister alongside the other handlers:
...
- [x] **New** `src/hooks/__tests__/useMapInteractions.test.tsx` — follow the fake-map pattern from
...
- [x] `npx vitest run src/hooks/__tests__/useMapInteractions.test.tsx` → new file green
- [x] Browser sanity (dev server): cold load →
      `document.documentElement.scrollHeight === window.innerHeight` → `true`
- [x] Commit: `fix(map): hover tooltip no longer overflows the page or lingers across camera moves`

## Task 2 — search: Enter commits the top result

- [x] `src/components/SearchBar.tsx` — in the results effect, replace `setActiveIndex(-1)` with:
...
- [x] Add `spellCheck={false}` to the `<input>` attributes.
- [x] **New** `src/components/__tests__/SearchBar.test.tsx`: render with fixture countries
...
- [x] `npx vitest run src/components/__tests__/SearchBar.test.tsx` → green
- [x] Commit: `feat(search): Enter commits the top result; disable input spellcheck`

## Task 3 — cap highlight extrusions at z6

- [x] `src/lib/mapLayers.ts` — add `maxzoom: 6` to `country-extrusion` (in `addHoverLayers`) and to
...
- [x] **New** `src/lib/__tests__/mapLayers.test.ts` — fake map capturing `addLayer` specs; call
      `addHoverLayers` / `addSelectionLayers` / `addCompareLayers`; assert every
      `fill-extrusion`-type spec has `maxzoom: 6`.
- [x] `npx vitest run src/lib/__tests__/mapLayers.test.ts` → green
- [x] Commit: `fix(map): cap highlight extrusions at z6 to prevent high-zoom walls`

## Task 4 — flyToCountry: retuned zoom + preserveZoom option + panel-aware offset

- [x] `src/lib/flyToCountry.ts` — full replacement:
...
- [x] Rewrite `src/lib/__tests__/flyToCountry.test.ts` (tolerance assertions, jsdom `matchMedia`
...
- [x] `npx vitest run src/lib/__tests__/flyToCountry.test.ts` → green
- [x] Commit: `feat(map): retuned selection zoom with panel-aware offset and preserveZoom option`

## Task 5 — origin plumbing (click vs auto)

- [x] **New** `src/lib/selectionOrigin.ts`:
...
- [x] `src/hooks/useMapInteractions.ts` — in `clickCountry`, before `onSelectRef.current(...)`:
...
- [x] `src/hooks/useSelectedCountry.ts` — add `const selectionOriginRef = useRef<'click' | 'auto'>('auto')`;
...
- [x] `src/App.tsx` — destructure `selectionOriginRef` and pass to `<WorldMap …>`.
- [x] `src/components/WorldMap.tsx` — accept prop, pass into `useSelectionHighlight`.
- [x] `src/hooks/useSelectionHighlight.ts`:
...
- [x] Update `src/hooks/__tests__/useSelectionHighlight.test.tsx` for the new prop/arguments; add
      a case: origin ref `'click'` → `flyToCountry` called with `preserveZoom: true`.
- [x] `npx vitest run src/hooks src/lib` → green
- [x] Commit: `feat(map): map clicks keep zoom, auto selections may zoom out (origin-gated camera)`

## Task 6 — verification (batch)

- [x] `npm run check` → lint, typecheck, unit all green
- [x] Kill stray servers, then
      `npx playwright test search.spec.ts map-and-countries.spec.ts --project=chromium` → all pass
- [x] Live pass (dev server, fresh profile):
...
- [x] Update spec Status → Accepted; commit any test/doc touch-ups
- [x] On merge: move this plan to `docs/superpowers/plans/archive/`

## Execution notes (2026-07-10)

- All tasks executed inline; `npm run check` green (336 tests), targeted e2e
  `search.spec.ts` + `map-and-countries.spec.ts` 19/19 on chromium, live pass
  verified: no page scrollbars (desktop + mobile), `#VAT` clean at z≈11,
  Vatican → search-Japan zooms out and frames the archipelago left of the
  panel, Enter commits the top result, no stale tooltip after hash fly-to,
  world-zoom extrusion lift intact (Brazil), mobile offset centers above the
  sheet.
- Deviation 1: the movestart tests were merged into the EXISTING
  `useMapInteractions.test.ts` — a new same-basename `.tsx` sibling is
  silently dropped from the TS program (same-name dedup prefers `.ts`), which
  surfaced as a typescript-eslint "file not found in project" error.
- Deviation 2: an additional `overflow-hidden` on WorldMap's wrapper clips the
  tooltip at the map bounds — a stale hover position near an edge (or after a
  viewport shrink) could still stretch the document sideways despite the
  initial top/left fix.
- SearchBar unit tests wait for the auto-activation commit (`waitFor` on
  `aria-selected`) — asserting immediately after `findByRole` raced the
  activation effect under full-suite load.
