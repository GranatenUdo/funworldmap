...
- No-op when `window.__PLAYWRIGHT__` is set — a unit-test seam (`src/test/analyticsCapture.ts`) that captures events to `window.__testAnalytics`. E2e builds simply leave `VITE_ANALYTICS_ENDPOINT` empty, so `track()` is already a no-op there.
